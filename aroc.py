...
def aroc(features, n_neighbours, threshold, num_proc=4):
    ...
    # nbrs = NearestNeighbors(n_neighbors=n_neighbours, algorithm='auto',
    #                         metric=lambda a, b: paired_distances(np.asarray([a]), np.asarray([b]),
    #                                                              metric='cosine')).fit(features)
    nbrs = NearestNeighbors(n_neighbors=n_neighbours, algorithm='auto').fit(features)
    distances, nearest_neighbours = nbrs.kneighbors(features)
